module GamePlay
  ...
  class ArkMenu < BaseCleanUpdate
    ...
    def create_entering_animation
      ya = Yuki::Animation
      button_animations = @buttons.map { |button| ya.move_discreet(0.25, button, button.x, button.y, button.x - ENTERING_ANIMATION_OFFSET, button.y) }
      return ya.parallel(ya.opacity_change(0.25, @background, 0, 255), *button_animations).start
    end
    ...
    def create_quitting_animation
      ...
      ya = Yuki::Animation
      button_animations = @buttons.map { |button| ya.move_discreet(0.25, button, button.x, button.y, button.x + ENTERING_ANIMATION_OFFSET, button.y) }
      return ya.parallel(ya.opacity_change(0.25, @background, 255, 0), *button_animations).start
    end
    ...
    register_button(:open_questbook) { $game_switches[Yuki::Sw::QuestBook] }             
...
GamePlay.menu_class = GamePlay::ArkMenu